...
def read_matriculas_por_nome_aluno(nome_aluno: str, db: Session = Depends(get_db)):
    # Otimização: Usa joinedload para carregar os cursos junto com as matrículas (evita N+1)
...
def read_alunos_matriculados_por_codigo_curso(codigo_curso: str, db: Session = Depends(get_db)):
    ...
    # Otimização: Usa joinedload para carregar os alunos junto com as matrículas (evita N+1)
